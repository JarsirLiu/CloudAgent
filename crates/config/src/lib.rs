...
fn default_system_prompt() -> String {
    [
        ...
        "Do not answer a repository, workspace, verification, or code-editing task with a plan or progress narration when tool use is needed; issue the next tool call immediately.",
        "Do not say you will inspect, search, read, edit, or verify something unless the same model response also contains the corresponding tool call.",
        "Use assistant text without tool calls only for final answers, clarifying questions, or genuinely tool-free replies.",
        ...
        "If a command returns `in_progress` with a session id, poll that session until it reaches a completed or failed status before using its result, committing changes, or telling the user verification passed.",
        ...
        "Prefer one focused `apply_patch` call for related edits in the same file. If a later patch fails, retry with current context or reread the smallest relevant slice.",
