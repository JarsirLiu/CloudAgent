...
impl ServerState {
    pub(crate) fn new(default_session_id: String) -> Self {
        Self {
            active_session_id: default_session_id.clone(),
            subscriptions: ConversationSubscriptions::new(default_session_id),
    ...
    pub(crate) fn set_active_listener(
        ...
        self.active_listeners.insert(session_id, listener);
    }

    pub(crate) fn clear_active_listener(&mut self, session_id: &str) {
        self.active_listeners.remove(session_id);
    }
    ...
    pub(crate) fn active_listener(
        ...
        self.active_listeners.get(session_id).cloned()
    }
    ...
    pub(crate) fn switch_active_conversation(&mut self, session_id: String) {
        self.active_session_id = session_id;
    }

    pub(crate) fn is_subscribed(&self, session_id: &str) -> bool {
        self.subscriptions.is_subscribed(session_id)
    }

    pub(crate) fn subscribe(&mut self, session_id: String) {
        self.subscriptions.subscribe(session_id);
    }

    pub(crate) fn unsubscribe(&mut self, session_id: &str) {
        self.subscriptions.unsubscribe(session_id);
    }
    ...
    pub(crate) fn drain_server_requests_for_conversation(
        ...
        self.server_requests
            .drain_conversation(session_id, decision)
    }
    ...
    pub(crate) fn pending_server_requests_for_conversation(
        ...
        self.server_requests.pending_for_conversation(session_id)
    }
    ...
    pub(crate) fn insert_pending_server_request(
        ...
        self.server_requests
            .insert_pending(request_id, session_id, turn_id, request, reply_tx);
    }
}
...
pub(crate) async fn handle_command(
    ...
    match command {
        ...
        AppClientCommand::InterruptTurn { session_id } => {
            turn_service::interrupt_turn(&runtime, event_tx, &state, session_id).await;
        }
        AppClientCommand::CompactSession { session_id } => {
            turn_service::compact_conversation(&runtime, event_tx, &state, session_id).await?;
        }
        ...
        AppClientCommand::SwitchSession { session_id } => {
            conversation_service::switch_conversation(&runtime, event_tx, &state, session_id)
                .await?;
        }
        AppClientCommand::ArchiveSession { session_id } => {
            conversation_service::archive_conversation(&runtime, event_tx, &state, session_id)
                .await?;
        }
        AppClientCommand::ResetSession { session_id } => {
            conversation_service::reset_conversation(&runtime, event_tx, &state, session_id)
                .await?;
        }
        AppClientCommand::SubscribeSession { session_id } => {
            conversation_service::subscribe_conversation(event_tx, &state, session_id.clone())
                .await;
            server_request_service::replay_pending_for_conversation(
                event_tx,
                &state,
        ...
        AppClientCommand::UnsubscribeSession { session_id } => {
            conversation_service::unsubscribe_conversation(event_tx, &state, session_id).await;
        }
